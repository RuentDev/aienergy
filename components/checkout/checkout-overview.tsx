...
import type { CheckoutData, CheckoutStep } from "@/app/checkout/page"
...
export function CheckoutOverview({
  ...
  const currentPickupTimes = pickupTimes[warehouse as keyof typeof pickupTimes] || pickupTimes.Sydney
    ...
    return (
      <div className="text-center py-16 animate-fade-in-up">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-muted">
          ...
          <span className="font-mono font-bold text-accent">#AES-{Date.now().toString().slice(-8)}</span>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6 animate-fade-in-up">
        ...
        <CardContent className="pt-6">
          ...
          {checkoutData.voucherCode && (
            <div className="mt-4 p-3 rounded-lg bg-accent/10 border border-accent/20 flex items-center gap-2">
              ...
              <Badge className="bg-accent text-white">{checkoutData.voucherCode}</Badge>
            </div>
          )}
          ...
          {checkoutData.orderNotes && (
            <div className="mt-4 p-3 rounded-lg bg-muted/50 border border-border/50">
              ...
              <p className="text-sm text-muted-foreground">{checkoutData.orderNotes}</p>
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <CardHeader className="bg-gradient-to-r from-accent/10 to-transparent border-b border-border/50">
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              {checkoutData.deliveryMethod === "delivery" ? "Delivery Details" : "Pickup Details"}
            </CardTitle>
            <Button
              variant="ghost"
              size="sm"
              ...
              <div className="p-4 rounded-xl bg-muted/30 border border-border/50">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">{checkoutData.shippingAddress.address}</p>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-muted/30 border border-border/50">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {warehouseAddresses[warehouse as keyof typeof warehouseAddresses]}
                    </p>
                  </div>
                </div>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-accent/5 border border-accent/20">
                  ...
                  <p className="text-accent font-semibold">
                    {checkoutData.pickupDate ? format(checkoutData.pickupDate, "EEEE, MMMM d, yyyy") : "Not selected"}
                  </p>
                </div>
                <div className="p-4 rounded-xl bg-accent/5 border border-accent/20">
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-accent font-semibold capitalize">
                    {checkoutData.pickupTime} ({currentPickupTimes[checkoutData.pickupTime]})
                  </p>
                </div>
              </div>
              {checkoutData.pickupNotes && (
                <div className="p-4 rounded-xl bg-muted/50 border border-border/50">
                  ...
                  <p className="text-sm text-muted-foreground">{checkoutData.pickupNotes}</p>
                </div>
              )}
            </div>
          )}
      ...
      <Card className="border-border/50 shadow-lg overflow-hidden">
        <CardHeader className="bg-gradient-to-r from-accent/10 to-transparent border-b border-border/50">
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              {checkoutData.paymentMethod === "credit-card" && <CreditCard className="w-5 h-5 text-accent" />}
              {checkoutData.paymentMethod === "bank-transfer" && <Building2 className="w-5 h-5 text-accent" />}
              {checkoutData.paymentMethod === "account-credit" && <Wallet className="w-5 h-5 text-accent" />}
              Payment Method
            </CardTitle>
            <Button
              variant="ghost"
            ...
            {checkoutData.paymentMethod === "bank-transfer" && (
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Payment details will be sent via email</p>
                </div>
              </div>
            )}
            {checkoutData.paymentMethod === "account-credit" && (
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Using store credit balance</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">
                {checkoutData.deliveryMethod === "delivery" ? "Shipping" : "Pickup"}
              </span>
            ...
            <div className="flex justify-between items-center pt-2">
              ...
              <span className="font-bold text-3xl text-accent">${finalTotal.toFixed(2)}</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <Button variant="outline" onClick={onBack} className="h-12 px-6 gap-2 bg-transparent hover:bg-muted group">
