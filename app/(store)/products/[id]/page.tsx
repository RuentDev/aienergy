      ...
      <div className="bg-muted/20 border-b border-border">
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm flex-wrap">
            ...
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
            <Link href={`/products?search=${product.category}`} className="hover:text-accent transition-colors">
              {product.category}
            </Link>
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
                      ...
                      <>
                        ...
                        {product.powerRating && (
                          <div className="flex justify-between p-3 bg-muted/30 rounded-lg">
                            ...
                            <span className="text-muted-foreground">{product.powerRating}</span>
                          </div>
                        )}
                        {product.phaseSupport && (
                          <div className="flex justify-between p-3 bg-muted/30 rounded-lg">
                            <span className="font-medium">Phase Support:</span>
                            <span className="text-muted-foreground">{product.phaseSupport}</span>
                          </div>
                        )}
                        {product.inverterType && (
                          <div className="flex justify-between p-3 bg-muted/30 rounded-lg">
                            <span className="font-medium">Inverter Type:</span>
                            <span className="text-muted-foreground">{product.inverterType}</span>
                          </div>
                        )}
                      </>
                    )}
                ...
                <Link key={relatedProduct.id} href={`/products/${relatedProduct.id}`}>
                  <Card className="group cursor-pointer hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden border-border hover:border-accent">
                    <div className="relative h-48 overflow-hidden bg-secondary">
                      ...
                      />
                      <Badge className="absolute top-3 right-3 bg-accent text-white">{relatedProduct.brand}</Badge>
                    </div>
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-lg font-bold text-accent">${relatedProduct.price}</span>
